...
class _CategoryPageState extends State<CategoryPage> {
  ...
  void initState()  {
    super.initState();
    _firebaseMessaging.subscribeToTopic(Resources().userProfile.email.replaceAll("@", '.'));
    ...
    subscription = eventBus.on<OrderChanged>().listen((event) {
      setState(() =>
          Scaffold.of(context).showSnackBar(MySnackBar.build(context:scaffoldKey.currentContext , message:event.notification["notification"]["body"],type:SnackBatMessageType.info,onTap: (){
            print(event.notification);
            Navigator.push(
                scaffoldKey.currentContext,
                MaterialPageRoute( builder: (context) => OrdersPage(openOrder:int.parse(event.notification["data"]["orderId"]) ,)));
          })));
        ...
        itemBuilder: (context, index) {
          return Column(
              children: <Widget>[
                ...
                widget.parentCategoryID == 0 && index == 0?DiscountList(height: 84,):Container(),
                Container(
