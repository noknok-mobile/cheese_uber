...
class ProfileEditController implements Routable {
    public function post() {
        ...
        if (!empty($files['photo'])) {
            ...
            if(!empty($postJson["props"])){
                while($arFields = $postJson["PROPS"]->fetch()) {
                    $userProps = \CSaleOrderUserPropsValue
                        ::GetList(
                            array("DATE_UPDATE" => "DESC"),
                            array('USER_PROPS_ID' => $postJson["ID"]),
                            false,
                            false,
                    ...
                    while($values =$userProps->fetch()) {
                        ...
                        if ($postJson["PROPS"][$values["ORDER_PROPS_ID"]]!=null) {
                            $values["VALUE"] = $postJson["PROPS"][$values["ORDER_PROPS_ID"]];
                            CSaleOrderUserPropsValue::Update($values);
                        }
                    }
